...
class TrajectoryFeatureExtractor:
    ...
    def __init__(self, feature_types=None):
        ...
        self.available_features = [
            'spatial', 'speed', 'direction', 'length', 'duration',
            'turning_points', 'speed_change'
        ]
    ...
    def extract_features(self, trajectory):
        ...
        # 提取速度特征
        if 'speed' in self.feature_types:
            mean_speed = np.mean(trajectory.sog) if len(trajectory.sog) > 0 else 0
            max_speed = np.max(trajectory.sog) if len(trajectory.sog) > 0 else 0
            min_speed = np.min(trajectory.sog) if len(trajectory.sog) > 0 else 0
            std_speed = np.std(trajectory.sog) if len(trajectory.sog) > 0 else 0
            features.extend([mean_speed, max_speed, min_speed, std_speed])
        ...
        if 'direction' in self.feature_types:
            if len(trajectory.cog) > 0:
                ...
                mean_direction = np.degrees(np.arctan2(sin_sum, cos_sum)) % 360
                ...
                mean_direction, mean_direction_change, max_direction_change = 0, 0, 0
            features.extend([mean_direction, mean_direction_change, max_direction_change])
...
def main():
    ...
    # 5. 特征提取
    print("正在提取轨迹特征...")
    feature_types=['spatial', 'speed', 'direction']     # 'spatial', 'speed', 'direction', 'length', 'duration', 'turning_points', 'speed_change'
